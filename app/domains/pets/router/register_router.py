...
def check_pet_search_id(
    ...
    if not re.fullmatch(r"[A-Za-z0-9]{8}", pet_search_id):
        return pet_error("PET_400_5", f"/api/v1/pets/check/{pet_search_id}")
    
    pet_repo = PetRepository(db)
    if pet_repo.exists_pet_search_id(pet_search_id):
        return pet_error("PET_409_1", f"/api/v1/pets/check/{pet_search_id}")
...
@router.post(
    ...
    responses=PET_REGISTER_RESPONSES,
)
def register_pet(
    request: Request,
...
@router.patch(
    ...
    responses=PET_UPDATE_RESPONSES,
)
def update_pet(
    pet_id: int,
    request: Request,
...
@router.delete(
    ...
    status_code=200,
    responses=PET_DELETE_RESPONSES,
)
def delete_pet(
    pet_id: int,
    request: Request,
